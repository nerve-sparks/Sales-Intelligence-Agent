...
the outbound Serper/LLM/Nexus network calls, and only in the tests that
...
Coverage (mapped to the brief's item-25 checklist):
  ...
  - Serper unavailable                          -> test_serper_unavailable_completes_with_warning
  ...
  - signal pages read Serper-derived events      -> test_signal_directory_reads_buying_events
...
async def test_serper_unavailable_completes_with_warning(org_ctx, make_company, monkeypatch):
    organisation_id, workspace_id = org_ctx
    company = await make_company()
    monkeypatch.setattr(serper_client, "is_configured", lambda: False)
    ...
    assert any("Serper" in w for w in refreshed.processing_warnings)
...
# ---------------------------------------------------------------------------
# Serper/LLM failure handling at the research layer
# ---------------------------------------------------------------------------
...
def _patch_serper_ok(monkeypatch, items: list[dict]):
    async def _search_news_and_site(_domain):
        return [{"link": it["url"], **it} for it in items], []

    monkeypatch.setattr(serper_client, "is_configured", lambda: True)
    monkeypatch.setattr(serper_client, "search_news_and_site", _search_news_and_site)
    monkeypatch.setattr(serper_client, "build_news_query", lambda domain: f"news:{domain}")
    monkeypatch.setattr(serper_client, "build_site_query", lambda domain: f"site:{domain}")
    monkeypatch.setattr(serper_client, "is_relevant", lambda *_a, **_kw: True)
    monkeypatch.setattr(serper_client, "match_confidence", lambda *_a, **_kw: 0.9)
    ...
    monkeypatch.setattr(serper_client, "to_evidence", _to_evidence)


async def test_llm_unavailable_marks_research_failed(monkeypatch):
    _patch_serper_ok(monkeypatch, [_fake_evidence_item(0)])
    ...
    assert summary["serper_failed"] is False
...
async def test_partial_llm_batch_failure(monkeypatch):
    ...
    _patch_serper_ok(monkeypatch, items)
...
# ---------------------------------------------------------------------------
# Signal Intelligence reads Serper-derived BuyingEvents
# ---------------------------------------------------------------------------
