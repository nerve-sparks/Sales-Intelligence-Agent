...
Auth: `/` login, `/forgot-password`, `/mfa-verification` (all in `features/auth/LoginPage.tsx`); `/onboarding`. Auth screens are sized to the viewport so they don't scroll on laptops — `main` is `h-screen overflow-hidden` (form column gets the scroll safety-net), vertical dims use `vh`-keyed `clamp()`/`min()`, and the trust badges hide under `[@media(max-height:820px)]`. The ambient globe image is pinned to the **page** bottom-left (not the content column) with a `vh`-capped height, so it scales down and stays low instead of climbing into the headline/features.
...
- List rows / cards / page-tabs navigate via `window.location.href`. Trigger Editor creates real `TriggerDefinition` rows via `POST /workspaces/{id}/triggers` (name + signal_types/signal_categories - see `lib/signalCategories.ts` for the real category/type values) → shown on Trigger Library via `GET .../triggers`. No status/priority/description/actions exist on the backend model, so the UI doesn't pretend to have them.
